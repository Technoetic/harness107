...
가능한 경우 현재 단계 범위의 Biome 설치·버전 확인·구성 보존 또는 초기화만 독립적인 도구 준비 작업자 역할에 위임한다.
작업자는 기존 구성 바이트를 보존하고 필요한 경우에만 제한된 설치, 초기화 및 버전 명령을 수행한다.
검증자 역할은 작업자 결과를 그대로 수락하지 않고 명령 종료 코드, 실제 버전, 구성 상태와 보고서를 확인한다.
